...
public class CoinCounter {
    ...
    private HashSet denominationHash;
    ...
    boolean equalsDenominations(int totalSum){
        for(int coinValue : this.denominations){
            if(totalSum == coinValue){
                return true;
            }
    ...
    int recursiveNumberOfCoinsRequired(int totalSum) {
        ...
        if(totalSum==0){
            ...
            if (equalsDenominations(totalSum)){
                return 1;
            }else{
                if(totalSum<0){
        ...
        int minNumberOfCoins = CHANGE_NOT_POSSIBLE_FLAG;
        for (int i=0; i<denominations.length; i++) {
            if(denominations[i]<totalSum){
                minNumberOfCoins = Math.min(minNumberOfCoins, recursiveNumberOfCoinsRequired(totalSum-denominations[i])+1);
            }
        }
